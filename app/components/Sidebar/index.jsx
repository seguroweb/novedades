...
const { LINKS_MENU } = require("@/app/constants/menuItems");
...
const Sidebar = () => {
  ...
  return (
    <div className="group fixed top-16 left-0 h-[calc(100vh-64px)] z-40">
      <div
        className={`
          w-16 group-hover:w-64 transition-all duration-300 ease-in-out 
            ...
            {LINKS_MENU.map((item) => (
              <div key={item.name}>
                {item.submenu ? (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <item.icon className="h-5 w-5 min-w-5 min-h-5 dark:text-gray-400" />
                      <span className="whitespace-nowrap hidden group-hover:inline">
                        {item.name}
                      </span>
                    </div>
